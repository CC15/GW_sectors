...
#    grid.append(np.linspace(event_time - sbe[i], event_time + (2.0 - sbe[i]), int(2048 * 0.4)))
    grid.append(np.linspace(event_time - 0.20, event_time + 0.05, 350))
for i in range(25):
#    grid.append(np.linspace(event_time - sbe[i], event_time + (2.0 - sbe[i]), int(2048 * 0.4)))
    grid_test.append(np.linspace(event_time - 0.20, event_time + 0.05, 350))
...
for i in range(100000):
    a = h1_new[i]
    for j in range(350):
        h1_signal_col.append(a[j])

h1_signal_col_pure = []
for i in range(100000):
    b = h1_pure_new[i]
    for j in range(350):
        h1_signal_col_pure.append(b[j])

h1_signal_col_test = []
for i in range(25):
    c = h1_new[i]
    for j in range(350):
        h1_signal_col_test.append(c[j])

h1_signal_col_test_pure = []
for i in range(25):
    d = h1_pure_new[i]
    for j in range(350):
        h1_signal_col_test_pure.append(d[j])
...
h1_signal_col = np.asarray(h1_signal_col)
h1_signal_col_pure = np.asarray(h1_signal_col_pure)
h1_signal_col_test = np.asarray(h1_signal_col_test)
h1_signal_col_test_pure = np.asarray(h1_signal_col_test_pure)
...
def create_segments_and_labels_train_noisy(df_func, time_steps, step, label_name):
    ...
    for i in range(0, len(df_func) - time_steps, step):
        xs = df_func["H1_Signal"].values[i: i + time_steps]
...
num_time_periods, num_sensors = 350, 1

num_time_periods_test, num_sensors_test = 350, 1
...
model = Sequential()
model.add(Reshape((350,1), input_shape=(input_shape,)))
model.add(Conv1D(32,16, padding='same', activation='tanh', input_shape=(350,1)))
