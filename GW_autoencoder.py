...
new_pure = df[['h1_signal']].copy() 
new_test_pure = df_test[['h1_signal']].copy()
...
h1_new = normalize(h1)
h1_test_new = normalize_test(h1_test)

h1_pure_new = normalize(h1_pure)
h1_test_pure_new = normalize_test(h1_test_pure)
...
h1_signal_col = np.hstack(h1_new)
#l1_signal_col = np.hstack(l1)
#v1_signal_col = np.hstack(v1) #hstack is used for creating a list of lists into a numpy array.
h1_signal_col_test = np.hstack(h1_test_new)

#Creating h1_signal, l1_signal and v1_signal columns 
h1_signal_col_pure = np.hstack(h1_pure_new)
#l1_signal_col = np.hstack(l1)
#v1_signal_col = np.hstack(v1) #hstack is used for creating a list of lists into a numpy array.
h1_signal_col_test_pure = np.hstack(h1_test_pure_new)
...
d_pure = {"H1_Signal": h1_signal_col_pure, "Sector": sector, "Timestamp": timestamp}

d_test_pure = {"H1_Signal": h1_signal_col_test_pure, "Sector": sector_test, "Timestamp": timestamp_test}
...
data_h1 = data.iloc[:,0] #Extracting h1_signal from 'new' dataframe
#l1 = new.iloc[:,1] #Extracting l1_signal from 'new' dataframe

#v1 = new.iloc[:,2] #Extracting v1_signal from 'new' dataframe
data_h1_test = data_test.iloc[0:25,0]

data_h1_pure = new_pure.iloc[:,0] #Extracting h1_signal from 'new' dataframe
#l1 = new.iloc[:,1] #Extracting l1_signal from 'new' dataframe
#v1 = new.iloc[:,2] #Extracting v1_signal from 'new' dataframe
h1_test_pure = new_test_pure.iloc[0:25,0]
...
data_pure = data_pure.round({"H1_Signal": 6})
data_test_pure = data_test_pure.round({"H1_Signal": 6})


LABEL = "Sector"
def create_segments_and_labels_train_noisy(df, time_steps, step, label_name):
...
LABEL = "Sector"
def create_segments_and_labels_test_noisy(df, time_steps, step, label_name):
...
LABEL = "Sector"
def create_segments_and_labels_train_pure(df, time_steps, step, label_name):
    ...
    labels = []
    for i in range(0, len(df) - time_steps, step):
        xs = data_pure["H1_Signal"].values[i: i + time_steps]
#        ys = data["L1_Signal"].values[i: i + time_steps]
#        zs = data["V1_Signal"].values[i: i + time_steps]
        # Retrieve the most often used label in this segment
        label = stats.mode(df[label_name][i: i + time_steps])[0][0]
...
LABEL = "Sector"
def create_segments_and_labels_test_pure(df, time_steps, step, label_name):
    ...
    labels = []
    for i in range(0, len(df) - time_steps, step):
        xs = data_test_pure["H1_Signal"].values[i: i + time_steps]
#        ys = data["L1_Signal"].values[i: i + time_steps]
#        zs = data["V1_Signal"].values[i: i + time_steps]
        # Retrieve the most often used label in this segment
        label = stats.mode(df[label_name][i: i + time_steps])[0][0]
...
input_sig = Input(batch_shape=(None,512,1))
x = Conv1D(64,16, padding='same', activation='tanh', name='conv_1')(input_sig)
x = MaxPooling1D(pool_size=2, padding='same')(x)
x = Conv1D(32,16, padding='same', activation='tanh', name='conv_2')(x)
x = MaxPooling1D(pool_size=2, padding='same')(x)
x = Conv1D(16,16, padding='same', activation='tanh', name='conv_3')(x)
x = MaxPooling1D(pool_size=2, padding='same')(x)
...
x = Conv1D(16,16, padding='same', activation='tanh', name='conv_6')(x)
x = UpSampling1D(size=2)(x)
x = Conv1D(32,16, padding='same', activation='tanh', name='conv_7')(x)
x = UpSampling1D(size=2)(x)
x = Conv1D(64,16, padding='same', activation='tanh', name='conv_8')(x)
x = UpSampling1D(size=2)(x)
...
for i in range(24):
    np.savetxt("Prediction_files/Preds_"+str(i)+".txt", np.transpose(decoded_signals[i]))
    
for i in range(24):
    np.savetxt("Pure_signal_files/Preds_"+str(i)+".txt", np.transpose(x_test_pure[i]))

for i in range(24):
    np.savetxt("Noisy_signal_files/Preds_"+str(i)+".txt", np.transpose(x_test_noisy[i]))
 
    
#p = np.loadtxt("Prediction_files/Preds_0.txt")
#plt.plot(p)
#plt.savefig('Decoded_signal.png', dpi=150)

#p1 = np.loadtxt("Prediction_files/Preds_0.txt")
#plt.plot(p1)
#plt.plot(x_test_pure[0])
#plt.savefig('Decoded_signal+pure_signal.png', dpi=150)
